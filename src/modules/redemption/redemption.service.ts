...
export class RedemptionService {
  ...
  // Xác nhận sử dụng voucher → cập nhật USED (BR-PAR-06)
  async redeemVoucherCode(ma_voucher_code: string, ma_cn: string) {
  ...
  async getPartnerStats(ma_dt: string) {
    ...
    const tongDoanhThu = vouchers.reduce((sum, v) => {
      return sum + v.gia_ban * v.so_luong_da_ban;
    }, 0);
  ...
  async getAdminStats() {
    ...
    const tongDoanhThu = orders
      .filter((o) => o.trang_thai_thanh_toan === 'PAID')
