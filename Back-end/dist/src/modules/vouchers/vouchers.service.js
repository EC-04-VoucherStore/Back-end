    ...
    async searchVouchers(dto) {
        ...
        if (dto.ma_cn) {
            const { data: voucherBranches, error: branchError } = await client
                ...
                .eq('ma_cn', dto.ma_cn);
            if (branchError)
                throw new common_1.NotFoundException(branchError.message);
            voucherIdsByBranch = [
        ...
        if (keyword) {
            ...
            if (matchedBranchError)
                throw new common_1.NotFoundException(matchedBranchError.message);
            ...
            if (matchedBranchIds.length > 0) {
                ...
                if (matchedVoucherBranchError)
                    throw new common_1.NotFoundException(matchedVoucherBranchError.message);
        ...
        let results = (data ?? []).filter((voucher) => {
            ...
            if (soLuongPhatHanh <= soLuongDaBan)
                return false;
            if (keyword) {
                ...
                const matchPartnerName = String(voucher.doi_tac?.ten_doanh_nghiep ?? '')
                    .toLowerCase()
                    .includes(normalizedKeyword);
                const matchBranchName = voucherIdsByKeywordBranch.includes(voucher.ma_voucher);
                if (!matchVoucherName && !matchPartnerName && !matchBranchName)
                    return false;
            }
            if (dto.giam_min) {
                if (!giaGoc || giaGoc <= 0)
